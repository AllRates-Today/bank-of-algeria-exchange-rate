# Bank of Algeria Exchange Rates API — bank-of-algeria-exchange-rate

[![npm version](https://img.shields.io/npm/v/bank-of-algeria-exchange-rate.svg)](https://www.npmjs.com/package/bank-of-algeria-exchange-rate)
...
- 🏛️ **Official published rates** — Bank of Algeria's own table, with the publisher's own `rate_date` on every response
...
> **Official rate, not mid-market:** every value here is a number Bank of Algeria itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Bank of Algeria table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/bda?source=USD&target=DZD"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/bda').then((x) => x.json());
...
```bash
yarn add bank-of-algeria-exchange-rate
```

```bash
pnpm add bank-of-algeria-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/bank-of-algeria-exchange-rate`](https://www.npmjs.com/package/@allratestoday/bank-of-algeria-exchange-rate) — same code, same versions.
...
console.log(pair.rate, pair.rate_date); // the official Bank of Algeria rate, on the central bank's own date
```
...
```javascript
{
  ...
  rate_date: '2026-09-09',   // Bank of Algeria's own publication date
  source: 'USD',
  target: 'DZD',
  rate: 133.0459,
  rate_type: 'reference',
...
```js
import { getLatestRates } from 'bank-of-algeria-exchange-rate';
...
```javascript
{
  ...
  rates: [
    { "base": "USD", "quote": "DZD", "type": "reference", "value": 133.0459 },
...
```js
import { getRatesForDate } from 'bank-of-algeria-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'DZD' });
```
...
const series = await getHistory(
  { source: 'USD', target: 'DZD', from: '2026-01-01', to: '2026-09-09' },
  { apiKey: 'art_live_...' }
);
```
...
```javascript
{
  ...
  rates: [
    ...
    { date: '2026-09-09', rate: 133.0459, rate_type: 'reference', derived: false, method: 'published' },
    // …
  ],
  disclaimer: '…'
}
...
Bank of Algeria currently publishes rates covering **17 currencies** against the DZD (as of the latest table):

🇦🇪 `AED` · 🇨🇦 `CAD` · 🇨🇭 `CHF` · 🇨🇳 `CNY` · 🇩🇰 `DKK` · 🇪🇺 `EUR` · 🇬🇧 `GBP` · 🇯🇵 `JPY` · 🇰🇼 `KWD` · 🇱🇾 `LYD` · 🇲🇷 `MRU` · 🇳🇴 `NOK` · 🇸🇦 `SAR` · 🇸🇪 `SEK` · 🇹🇳 `TND` · 🇺🇸 `USD` · `XDR`
...
- Publisher's own page: [Taux de change journalier](https://www.bank-of-algeria.dz/taux-de-change-journalier/) · [www.bank-of-algeria.dz](https://www.bank-of-algeria.dz)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Bank of Algeria rates page](https://allratestoday.com/central-bank-rates-api/bda/)
...
- Bank of Algeria quotes **DZD per 1 unit of foreign currency** (e.g. `base: "USD", quote: "DZD"` means DZD per one US dollar).
...
Loading the official Bank of Algeria rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/bda/latest?format=xml&api_key=art_live_..."
```
...
If Bank of Algeria does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via DZD from two published rates |
...
| `404` | Pair or date range not covered by Bank of Algeria |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'bank-of-algeria-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('bank-of-algeria-exchange-rate');

getRate('USD', 'DZD', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- Hugging Face: [AllRates/central-bank-exchange-rates](https://huggingface.co/datasets/AllRates/central-bank-exchange-rates) — one CSV per institution (`rates/bda.csv`)
- Kaggle: [allratestoday/central-bank-exchange-rates](https://www.kaggle.com/datasets/allratestoday/central-bank-exchange-rates)
- CDN JSON: `https://cdn.jsdelivr.net/gh/AllRates-Today/central-bank-exchange-rates@main/data/bda/latest.json`

## 🔗 Links

- [Bank of Algeria rates page](https://allratestoday.com/central-bank-rates-api/bda/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/bank-of-algeria-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/bank-of-algeria-exchange-rate)
